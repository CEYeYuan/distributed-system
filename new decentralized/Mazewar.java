...
public class Mazewar extends JFrame {
         ...
         */
        private final static int mazeSeed = 0;
        private Random randomGen = new Random(mazeSeed); 
        /**
         * The {@link Maze} that the game uses.
         */
        private static Maze maze = new MazeImpl(new Point(mazeWidth, mazeHeight), mazeSeed);;
         ...
         * A queue of events.
         */
        private  static BlockingQueue eventQueue = new LinkedBlockingQueue<MPacket>();;
        ...
        public static void main(String args[]) throws IOException,
            ...
            try {
                if(args.length == 4) {
                    ...
                    new Thread(new PeerListenerDispatcher(serverSocket,out_map,args[3],mylocation,token,clientTable,seq, maze, eventQueue)).start();//listen to the connection from other peers
                   /***************************************
                   ...
                   while(true){
                        ...
                        if(packetFromServer.type==MPacket.TOKEN){
                        	...
                        	if (packetFromServer.first == 1){
                        		System.out.println("CREATE a xxxxxxxxxxxxxxxxxxxxxxxxxxxxxnew THREAD " + packetFromServer.symbol);
                        		(new Thread(new missileTick(args[3], token, eventQueue))).start();
                        		packetFromServer.first = 0;
                        	}
                            System.out.println("syncing done: Know "+count+" peers");
                        ...
                        if(out_map.get(packetFromServer.symbol)==null){
                            ...
                            new Thread(new PeerListener(out_map,packetFromServer.symbol,packetFromServer.location,args[3],mylocation,token,clientTable,seq, maze, eventQueue)) .start();
                            count++;
                        }
